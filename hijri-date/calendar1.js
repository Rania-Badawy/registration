...
function Calendar(isHijriMode, firstDayOfWeek, isAutoHide, isAutoSelectedDate, year, month, date,DbName) {
	...
	if(DbName!="SchoolAcclittlecaterpillars"){
		...
		var hijriDate = gregorianToHijri(adjustedDate.getFullYear(), adjustedDate.getMonth(), adjustedDate.getDate());
	}
	...
	var self = this,
		...
		createCalendar = function () {
			...
			nextMonthElement.classList.add('here');
			...
			yearElement.setAttribute('type', 'string');
			yearElement.setAttribute('max', '1441');
			yearElement.setAttribute('min', '1430');
			yearElement.setAttribute('readonly','');
			nextYearElement.classList.add('header-button');
			nextYearElement.classList.add('here');
		...
		createDateGrid = function () {
			...
			for (var i = 1; i <= cdim; i++) {
				...
				if (thisDate.getFullYear() === currentYear && thisDate.getMonth() === currentMonth && i === currentDate) de.classList.add('s');
		...
		updateCalendar = function () {
            ...
            selectedDate.setDate(currentDay > dim ? dim : currentDay);
		...
		onDecrementMonth = function (evt) {
			...
			for (const here of heres) {
				// 👇️ hides element
				if (thisDate.getFullYear() > 1333 && thisDate.getFullYear() < 1888) {
					if (thisDate.getFullYear() < hijriDate[0]  ) {
					...
					if (thisDate.getFullYear() < adjustedDate.getFullYear()) {
						here.style.visibility = 'visible';
					}
				}
