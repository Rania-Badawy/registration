...
day = (day + 4) % 7;
day = (day < 0) ? day + 7 : day; } function updateTime() { var months=(year - 1) * 12 + month; time=(months>= 0) ? HijriDate.daysCount(months) : -HijriDate.daysCount(months);
    time += date;
    time *= 864e5;
    ...
    HijriDate.monthNames = ["محرم", "صفر ", "ربيع الاول", "ربيع الاخر", "جماد الاول", "جماد الاخر",
    "رجب", "شعبان", "رمضان", "شوال", "ذوالقعدة", "ذو الحجة"];
    ...
    day = Math.floor(day);
    day = (day + 3) % 5;
    return (day < 0) ? day + 5 : day; }; Date.prototype.getJavaWeekdayName=function(day) { return Date.javaWeekdayNames[(day===undefined) ? this.getJavaWeekday() : parseInt(day)]; }; Date.prototype.getFullYearString=function() { var y=this.getFullYear(); return (y> 0) ? HijriDate.toDigit(y, 4) + " AD" : HijriDate.toDigit(Math.abs(y - 1), 4) + " BC";
        };
