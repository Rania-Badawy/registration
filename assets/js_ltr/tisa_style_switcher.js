// style switcher
	...
	$(function() {
		...
		'<style>'+
		'#style_switcher {position: fixed;z-index: 1000;top: 100px;left: -215px;margin-left: 0;padding: 10px;width: 191px;}'+
		...
		'#style_switcher .switcher_toggle {position: absolute;right: -30px;top: 40px;width: 30px;height: 30px;line-height: 30px;text-align: center;font-size: 19px;color: #555;background:#ddd;outline: none;}'+
		'#style_switcher ul {margin: 0;padding: 0;}'+
		'#style_switcher ul li {list-style: none;float: right;width: 26px;height: 26px;margin-bottom: 10px;text-indent: -9999px;cursor: pointer;-webkit-box-shadow: inset 0 0 0 1px rgba(0, 0, 0, 0.2);-moz-box-shadow: inset 0 0 0 1px rgba(0, 0, 0, 0.2);box-shadow: inset 0 0 0 1px rgba(0, 0, 0, 0.2);}'+
		'#style_switcher ul li + li {margin-right: 10px;}'+
		'#style_switcher ul li:nth-child(5n+6) {clear: right;margin-right: 0;}'+
		...
		'    <p class="style_title">شكل القائمة</p>'+
		...
		'    <p class="style_title">حجم القائمة</p>'+
		'    <label class="radio-inline">'+
		'      <input type="radio" name="navigation_size" id="navigation_regular" value="regular" checked> شكل كبير'+
		'    </label>'+
		'    <label class="radio-inline">'+
		'      <input type="radio" name="navigation_size" id="navigation_narrow" value="narrow"> شكل صغير'+
		'    </label>'+
		'  </div>'+
		...
		$('.switcher_toggle').on('click', function(e) {
			if(!$('#style_switcher').hasClass('switcher_open')) {
				$('#style_switcher').animate({ left: 0 },200, function() {
					$(this).addClass('switcher_open')  
				})  
			} else {
				$('#style_switcher').animate({ left: '-215px'},200, function() {
