...
// ########################################################################################################################
// ## Note: this is my attempt at having an auto width nav. It is not ready yet and needs much more time to make it work ##
// ## 		this script isn't working, and I don't think I'll be able to make it work easily, so I'm dropping it for now ##
// ########################################################################################################################
...
(function($){
	...
	$.fn.navPlugin.addClasses = function(myNav)
	{
     ...
     $.fn.navPlugin.addCss = function(myNav,opts)
     {
         ...
         myNav.find("li").css({"display":"block","position":"relative"});
         myNav.find("li").find("a").css({"display":"block"});
         myNav.find("li").find("ul").css({"margin-left":"0px","position":"absolute","top":"-1px","left":"0px","display":"block"});
         myNav.stack.css({"position":"relative"});
         myNav.stack.find("ul").css({"position":"absolute","top":"-1px","left":"0px"});
         //myNav.toprow.css({});
         myNav.toprow.css({"float":"right","padding-right":"8px","padding-left":"8px"});
         myNav.toprow.find("ul").css({"margin-top":(opts.itemHeight+6)+"px"});
         //myNav.toprow.find("ul").find
         myNav.toprow.find("ul").find("li").css({"position":"relative","display":"none","width":"100%","text-align":"center","float":"right"});
         myNav.toprow.find("ul").find("li").find("ul").css({"right":"100%","top":"0px","position":"absolute","margin-top":"0px"});
         //console.log(opts.itemWidth);
         myNav.toprow.find("ul").find("li").find("ul").find("li").css({"margin-top":"0px"});
         myNav.submenu.find("li").css({"display":"none"});

		myNav.toprow.children("ul").each(function(){
              var parentWidth = $(this).parent().width();
              console.log(parentWidth)
              $(this).css("width",parentWidth+"px");
          });
		myNav.toprow.find("ul").find("ul").each(function(){
              var parentWidth = $(this).parent().width();
