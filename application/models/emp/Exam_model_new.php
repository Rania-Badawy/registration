<?php
class Exam_Model_New  extends CI_Model 
 {
	 private $txt_exam ;
...
///create_token
 ...
 //get_exams_students
	 ...
	 {
		$idContact = (int)$this->session->userdata('id'); 
		$this->db->select('vw_test_select.test_ID,vw_test_select.test_Name,vw_test_select.subject_Name,vw_test_select.contact_Name,vw_test_select.row_level_Name,vw_test_select.config_semester_Name,vw_test_select.test_Degree,contact.Name as student_name,contact.ID as student_ID,sum(test_student.Degree)as student_Degree');
		$this->db->from('config_emp');	
		$this->db->join('vw_test_select', 'vw_test_select.Subject_emp_ID =config_emp.ID', 'INNER');
		$this->db->join('test_questions', 'vw_test_select.test_ID =test_questions.Test_ID', 'INNER');
		...
		$this->db->where('vw_test_select.IsActive',1);
		$this->db->group_by('vw_test_select.test_ID,contact.ID');
		$ResultExam = $this->db->get();			
		$NumRowResultExam = $ResultExam->num_rows() ; 
