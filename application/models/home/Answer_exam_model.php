...
///create_token
 ...
 //get_exam_subject
	 public function get_exam_subject($test_ID )
	 {
		...
		$this->db->select('  job_specializationsID	 ');


		$this->db->from('vw_test_emp_select  AS vw_test'); 
			...
			if($NumRowResultExam >0)
			  {
			   ...
			   return $ReturnExam['job_specializationsID']  ; 
 ...
 //get_exams
	 public function get_exams($Lang)
	 {
		...
		$this->db->select('vw_test.test_ID , vw_test.test_Name, vw_test.test_Description, vw_test.time_count, vw_test.contact_Name as teacher_Name, vw_test.subject_Name ');
	    ...
	    $this->db->from('emp_application');	
        $this->db->join('job_specializations', 'emp_application.subjectID = job_specializations.ID', 'INNER');
	    $this->db->join ('vw_test_emp_select AS vw_test', 'job_specializations.ID = vw_test.job_specializationsID' );	
		...
		$this->db->where('vw_test.IsActive',1);
	 	$this->db->where('emp_application.contactID',$idContact);
	//	$this->db->where('class_table.SchoolID', (int)$this->session->userdata('SchoolID'));
		$this->db->where('CURRENT_TIMESTAMP() BETWEEN test.date_from and test.date_to');
 		$ResultExam = $this->db->get();			 //print_r($this->db->last_query());exit();
		$NumRowResultExam = $ResultExam->num_rows() ; 
 ...
 //get_exam_no_rpt
	 public function get_exam_no_rpt($Lang)
	 {
		...
		$this->db->select('test.date_to,test_ID,test_Name,test_Description,test_attach_file,questions_content_ID,Degree,Question,questions_types_ID,questions_types_Name,test.time_count');
		$this->db->from('vw_test_question_select');	
		 $this->db->join('test', 'test.ID = vw_test_question_select.test_ID', 'INNER');
		$this->db->where('test_ID',$Data['ExamID']);
 ...
 public function get_student_class  ()
	...
	public function get_exam_adv()
	 {
		$idContact = (int)$this->session->userdata('id'); 
					
		$this->db->select('test.ID as test_ID , test.Name as test_Name, test.Description as test_Description, test.time_count,row_level.Level_Name,row_level.Row_Name ,test.subject_id,subject.Name AS subject_Name ');
	    $this->db->from('test');	
	    $this->db->join('subject', 'subject.ID = test.subject_id', 'left');	
        $this->db->join('emp_application', 'subject.ID_HR = emp_application.majors', 'INNER');
        $this->db->join('row_level', 'row_level.ID =test.RowLevelID', 'left');
		$this->db->group_by('test.ID');
		$this->db->where('test.IsActive',1);
