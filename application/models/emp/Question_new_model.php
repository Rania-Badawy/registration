<?php
class Question_New_Model  extends CI_Model 
 {
	 private $q_ID ;
	 private $q_txt ;
	 private $q_attach ; 
...
//get_question
	 ...
	 {
	     $Data['Questions_type']     = $this->db->query("SELECT questions_types_ID from questions_content where ID =  ".$q_ID." ")->row_array();
	     $type = $Data['Questions_type']['questions_types_ID'];
		$this->db->select('answers.Answer ,answers.attach , answers.Answer_correct , answers.ID AS answers_ID , questions_content.ID AS questions_content_ID , 
		                   questions_content.Degree , questions_content.Question, questions_content.degree_difficulty ,questions_content.Q_attach ,
		                   test.Name AS test_Name ,questions_types.Name,questions_types.ID AS questions_types_ID,questions_content.youtube_script , 
		                   questions_content.num_ques ');
		...
		$this->db->join('answers', 'answers.Questions_Content_ID = questions_content.ID', 'LEFT');
		$this->db->join('test', 'test.ID = test_questions.Test_ID', 'INNER');
		
        $this->db->where('questions_content.ID', $q_ID );
        $this->db->group_by('answers.ID' );
        $this->db->order_by("questions_content.num_ques", "ASC");
        if($type == 1){
	...
	public function insert_question()
	 {
			...
			$txt_question             = $this->input->post('txt_question');
			   ...
			   (string)$this->Token   = $this->create_token();	
			    $Question_num = $this->db->query("select num_ques from questions_content where test_id =$txt_exam_ID order by ID DESC limit 1")->row_array();
				if(empty($Question_num)){$num = 1;}else{$num = $Question_num['num_ques']+1;}
				$add_q = array(
...
// 	print_r($add_q);die;
			  ...
			  if($Insert_q && $Insert_q_t){
			      $this->db->select('sum(Degree) AS SumDegreeQ');
			 
			$this->db->from('questions_content');
			$this->db->where('test_id', (int)$txt_exam_ID );
			$ResultContactData = $this->db->get();			
			$NumRowResultContactData  = $ResultContactData->num_rows() ; 
				if($NumRowResultContactData >0)
			  {
			   ...
			   $exam_id=(int)$txt_exam_ID;
			  
		  $this->db->query("UPDATE send_box SET Degree =$Degree WHERE Test_ID =$exam_id");
			  }
			   ...
			   case '4':			   
					...
					$answer_txt = $this->input->post('answer_txt');  
						...
						while($num_answers_txt>=$count_ans){ 
 							$add_answer = array(
								 'Answer'                 =>trim((string)$answer_txt[$num_answers_txt]) ,
								 'Answer_correct'         =>1  ,
								 'Questions_Content_ID'   =>(string)$Insert_q ,
								 'Token'                  =>(string)$this->create_token(),
								 	 'test_id'                =>$txt_exam_ID
		   ...
		   switch($txt_Tquestion_ID){
			   ...
			   case '2':
				...
				while($count_Choices<$num_Choices){
 					 $txt_Choices         = $this->input->post('txt_Choices');
 					 $slct_Correct_Answer         = $this->input->post('slct_Correct_Answer');
 					 $txt_attach         = $this->input->post('images');
					 if($txt_Choices[$count_Choices]  != ""){
 					 $add_answer = array(
								 'Answer'                 => $txt_Choices[$count_Choices]  ,
								 'Answer_correct'         =>$slct_Correct_Answer[$count_Choices]  ,
								 'attach'         =>$txt_attach[$count_Choices]  ,
								 'Questions_Content_ID'   =>(string)$Insert_q ,
								 'Token'                  =>(string)$this->create_token()
								 );
...
//insert_del_question
	...
	public function del_qui($ID)
	{
	   //print_r($ID);die;
		...
		if($this->db->delete('answers',array('Questions_Content_ID' =>(int)$ID)))
		{
	 		// $this->db->delete('test_questions',array('Questions_ID'  =>(int)$ID));
			$this->db->delete('questions_content',array('ID'=>(int)$ID));
...
//insert_question
	 public function add_exist_question($question,$exam_ID)
	 {
	  ...
	  public function get_all_question_exam($SubjectID,$RowLevelID)
	{
	    
	    $id  =$this->session->userdata('id'); 
	    $SchoolID           =$this->session->userdata('SchoolID');
		$ResultExam = $this->db->query("SELECT DISTINCT  questions_types.Name,questions_types.ID AS questions_types_ID ,
		    questions_content.Degree,questions_content.Question,questions_content.Q_attach,questions_content.ID AS questions_content_ID ,test.subject_id,test.RowLevelID
		    ,GROUP_CONCAT( DISTINCT(CONCAT_WS('||', answers.Answer , answers.Answer_correct)) SEPARATOR ',,')AS answers ,test.Name AS test_name,contact.Name AS emp_name
		    from questions_content 
            INNER JOIN answers on questions_content.ID =answers.Questions_Content_ID 
            INNER JOIN questions_types on questions_types.ID =questions_content.questions_types_ID 
            INNER JOIN test on questions_content.Test_ID =test.ID 
            INNER JOIN contact ON test.empID=contact.ID
            where
    		...
    		AND test.IsActive=1
    		AND test.SchoolId=$SchoolID
            group by questions_content.ID limit 50")->result();
	 ...
	 public function get_all_answer_exam($SubjectID,$RowLevelID,$questions_content_ID)
	{
	    
	     $id  =$this->session->userdata('id'); 
	    
		$ResultExam = $this->db->query("SELECT DISTINCT  questions_types.Name,questions_types.ID AS questions_types_ID ,
		    questions_content.Degree,questions_content.Question,questions_content.Q_attach,questions_content.ID AS questions_content_ID,GROUP_CONCAT( CONCAT_WS('-', answers.Answer , 
		    answers.Answer_correct) SEPARATOR ',')AS answers 
		    from questions_content 
            INNER JOIN answers on questions_content.ID =answers.Questions_Content_ID 
            INNER JOIN questions_types on questions_types.ID =questions_content.questions_types_ID 
            INNER JOIN test on questions_content.Test_ID =test.ID 
    		...
    		AND test.RowLevelID=$RowLevelID
    		AND questions_content.ID=$questions_content_ID
            group by answers.ID")->result();
		if(sizeof($ResultExam)>0)
