...
///create_token
 ...
 //get_exams_students
	 ...
	 {
		$idContact = (int)$this->session->userdata('id'); 
		$this->db->select('vw_exercise_select.test_ID,vw_exercise_select.test_Name,vw_exercise_select.subject_Name,vw_exercise_select.contact_Name,vw_exercise_select.row_level_Name,vw_exercise_select.config_semester_Name,vw_exercise_select.test_Degree,contact.Name as student_name,contact.ID as student_ID,sum(test_student.Degree)as student_Degree');
		$this->db->from('config_emp');	
		$this->db->join('vw_exercise_select', 'vw_exercise_select.Subject_emp_ID =config_emp.ID', 'INNER');
		$this->db->join('test_questions', 'vw_exercise_select.test_ID =test_questions.Test_ID', 'INNER');
		...
		$this->db->where('vw_exercise_select.IsActive',1);
		$this->db->group_by('vw_exercise_select.test_ID,contact.ID');
		$ResultExam = $this->db->get();			
		$NumRowResultExam = $ResultExam->num_rows() ; 
 ...
 //get_exams_students_father
	 ...
	 {
		$idContact = (int)$this->session->userdata('id'); 
		$this->db->select('contact.Name as student_name,vw_exercise_select.test_ID,vw_exercise_select.test_Name,vw_exercise_select.subject_Name,vw_exercise_select.contact_Name,vw_exercise_select.row_level_Name,vw_exercise_select.config_semester_Name,vw_exercise_select.test_Degree,sum(test_student.Degree)as student_Degree');
		...
		$this->db->join('vw_exercise_select', 'vw_exercise_select.test_ID =test_questions.Test_ID', 'INNER');
		$this->db->where('student.Father_ID',$idContact);
		$this->db->group_by('vw_exercise_select.test_ID');
		$ResultExam = $this->db->get();			
		$NumRowResultExam = $ResultExam->num_rows() ; 
  ...
  ///add_exam
	 public function add_exam($data)
	 {
        ...
        $Insert_Exam =  $this->db->insert('test', $Add_exam); 
		 ...
		 add_notification( $this->db->insert_id(),8,0,(int)$this->slct_subject,0 ,0);
		if($Insert_Exam){
