...
class Answer_Exam_New extends MY_Student_Base_Controller {
	private $data = array() ;
    ...
    function __construct()
    {
		...
		$this->load->model(array('student/answer_exam_new_model','emp/subject_model','emp/exam_model','emp/config_model'));
	...
	{
		         $Data['exam_details']    = $this->answer_exam_new_model->get_exams((string) $this->data['Lang'] );

				 $this->load->student_template('exam/answer_exam_list',$Data);
	}
public function answer_exam_header()
	{
$subjectID= (int)$this->uri->segment(4);
		         $Data['exam_details']    = $this->answer_exam_new_model->get_exams_header((string) $this->data['Lang'],$subjectID );
...
//sees_answer
	public function sees_answer()
	{
		...
		if(isset($Data['exam_id'])&&$Data['exam_id']!=0 )
		{
			...
			redirect('/student/answer_exam_new/show_exam', 'refresh');
		}
...
public function show_exam()
	{
		 $Data['exam_details'] = $this->answer_exam_new_model->get_exam_no_rpt((string) $this->data['Lang'] );
		 $Data['q_details'] = $this->answer_exam_new_model->get_questions((string) $this->data['Lang'] );
		 $Data['q_answers'] = $this->answer_exam_new_model->get_answers((string) $this->data['Lang'] );
		 $this->load->student_template('exam/answer_exam',$Data);
	}
//correction_exam
		...
		while($count_types<=7){
			...
			switch($txt_type){
				case'1':
					...
					$q_old_id_1 =  array();
					if($ans_ID !=""){
						$degree_1      = (int) $this->answer_exam_new_model->check_answer($ans_ID , $q_ID);
					...
					$this->answer_exam_new_model->insert_s_degrees($q_old_id_1,$idContact);
				break;
				case'2':
				$data            =  $this->answer_exam_new_model->get_degree($q_ID);
				$dataCorrect     =  $this->answer_exam_new_model->get_answer_correct_count($q_ID);
				$dataAnswers     =  $this->answer_exam_new_model->get_answer_correct($q_ID);
				$for_count=0;
				...
				while($count_answers<=$countRows){
					...
					//end check empty answer*/
					$this->answer_exam_new_model->insert_s_degrees($q_old_id_2,$idContact);
				break;
				case'3':
					...
					$q_old_id_3 =  array();
					if($ans_ID !=""){
						$degree_3 =(int)$this->answer_exam_new_model->check_answer($ans_ID , $q_ID);
					...
					$this->answer_exam_new_model->insert_s_degrees($q_old_id_3,$idContact);
				break;
				case'4':
					$data            =  $this->answer_exam_new_model->get_degree($q_ID);
					$dataCorrect     =  $this->answer_exam_new_model->get_answer_correct_count($q_ID);
					...
					while($count_answers<=$countRows){
						...
						if($ans_txt!=""){
							$data_answer    =  $this->answer_exam_new_model->get_answer($q_ID);
							$count_for_each=1;
							$no_strt_ans="";
						...
						//end check empty answer*/
						$this->answer_exam_new_model->insert_s_degrees($q_old_id_4,$idContact);
				break;
				case'5':
					$data            =  $this->answer_exam_new_model->get_degree($q_ID);
					$dataCorrect     =  $this->answer_exam_new_model->get_answer_correct_count($q_ID);
					...
					while($count_answers<=$countRows){
						...
						//end check empty answer*/
						$this->answer_exam_new_model->insert_s_degrees($q_old_id_5,$idContact);
				break;
				case'6':
				    $case_6 =1;
					...
					$this->answer_exam_new_model->insert_upload_answer($txt_attach_f,$q_ID);
				break;
				case'7':
				    $q_old_id_7 =  array();
					...
					$crct_ans_t_7 +=(int)$this->answer_exam_new_model->check_del_answer($ans_ID , $q_ID,$all_answer,$count_q_z,$idContact);
			...
			$this->answer_exam_new_model->not_answer($q_old_id);
			if($crct_ans_t_6!=1){
					...
					$this->index($messa_flash);
					}else{
					         redirect('answer_exam');
						  }//end httprefer
