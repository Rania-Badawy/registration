...
class Answer_Homework extends MY_Student_Base_Controller
{
	private $data = array();
	...
	function __construct()
	{
		...
		$this->load->model(array('student/answer_homework_model', 'student/answer_exam_model', 'emp/subject_model', 'emp/exam_model', 'emp/config_model','admin/setting_model'));
	...
	{
		$Data['exam_details']    = $this->answer_homework_model->get_exams((string) $this->data['Lang']);
		$this->load->student_template('homework_list', $Data);
	}
	public function answer_homework_header1()
	{
		$subjectID = (int) $this->uri->segment(4);
		$Data['exam_details']    = $this->answer_homework_model->get_exams_header1((string) $this->data['Lang'], $subjectID);
		
		$this->load->student_template('homework_list_new', $Data);
			
	}
	public function answer_homework_header()
	{
	    $Data['date'] = $this->setting_model->converToTimezone();
		$subjectID    = (int) $this->uri->segment(4);
		$homeworkID   = (int) $this->uri->segment(5);
		$Data['task'] = $this->uri->segment(6);
		$Data['exam_details']    = $this->answer_homework_model->get_exams_header((string) $this->data['Lang'], $subjectID,$homeworkID,$Data['task']);
// 		print_r($this->db->last_query());
		$this->load->student_template('homework_list', $Data);
			
	}
		public function answer_homework_headerx()
	{
		$subjectID = (int) $this->uri->segment(4);
		$Data['exam_details']    = $this->answer_homework_model->get_exams_header((string) $this->data['Lang'], $subjectID);
...
// die();
	...
	public function sees_answer()
	{
		...
		if (isset($Data['exam_id']) && $Data['exam_id'] != 0) {
			...
			redirect('/student/answer_homework/show_exam', 'refresh');
		}
	}
	//show_exam
	public function show_exam()
	{
		$Data['exam_details'] = $this->answer_homework_model->get_exam_no_rpt((string) $this->data['Lang']);
		$Data['q_details'] = $this->answer_homework_model->get_questions((string) $this->data['Lang']);
		$Data['q_answers'] = $this->answer_homework_model->get_answers((string) $this->data['Lang']);
		$this->load->student_template('answer_homework', $Data);
	}
	//correction_exam
	public function correction_exam()
	{
		$idContact           = (int) $this->session->userdata('id');
		$subjectID =  $this->answer_homework_model->get_exam_subject($this->input->post("txt_test_ID"));
					...
					switch ($txt_type) {
						...
						case '2':
							...
							while ($count_answers <= $countRows) {
								...
								$ans_ID            = $this->input->post($ans_ID);
								if ($answer_student != "") {
									$answer_student = $answer_student + ' , ' + $ans_ID;
								} else {
									$answer_student = $ans_ID;
								}
								if ($ans_ID != "") {
						...
						case '4':
							...
							while ($count_answers <= $countRows) {
								...
								if ($answer_student != "") {
									$answer_student = $answer_student + ' , ' + $ans_txt;
								} else {
									$answer_student = $ans_txt;
								}
										...
										if ($count_for_each == $count_answers) {
											...
											foreach ($pieces as $rowPices) {
												...
												if (substr_count($rowPices, "%") == 2) {
													...
													if (strstr($ans_txt, $no_strt_ans) == true) {
						...
						case '5':
							...
							$countRows       = $count_q;
							$count_q_no++;// $data['countRows']; //em
							$count_answers = 1;
							while ($count_answers <= $countRows) {
$case_6 = 1;
							$txt_attach   = "answer_txt_upload_" . $count_q_z;
							$txt_attach_f = $_FILES[$txt_attach]['name'];
							$txt_attach_f_6[]=$txt_attach_f;
						  $txt_content='txt_content_'. $count_answers;
	                            $answer       = $this->input->post($txt_content); 
	                     $q_answers_id_6[]=$answer;
							...
							if (isset($q_old_id_6[$q_ID])) {
														 
														} else {
					
						 							    
						//	$this->answer_exam_model->insert_upload_answer(  $answer ,$txt_attach_f, $q_ID);
							$q_old_id_6[$q_ID] = $q_ID;
														}
						  ...
						  count($q_old_id_6);
						//if((count($q_old_id_6))==$count_answers)
						if($count_q_no==$count_q)
						{ 
						    //$this->answer_exam_model->insert_upload_answer(44 ,$txt_attach_f, $q_ID);
						    $this->answer_exam_model->insert_upload_answer_array($q_answers_id_6 ,$txt_attach_f_6, $q_old_id_6);
			...
			$q_old_id      =  array();
			if (isset($q_old_id_1)) {
				$q_old_id = array_merge($q_old_id, $q_old_id_1);
			}
			if (isset($q_old_id_2)) {
				$q_old_id = array_merge($q_old_id, $q_old_id_2);
			}
			if (isset($q_old_id_3)) {
				$q_old_id = array_merge($q_old_id, $q_old_id_3);
			}
			if (isset($q_old_id_4)) {
				$q_old_id = array_merge($q_old_id, $q_old_id_4);
			}
			if (isset($q_old_id_5)) {
				$q_old_id = array_merge($q_old_id, $q_old_id_5);
			}
			if (isset($q_old_id_7)) {
				$q_old_id = array_merge($q_old_id, $q_old_id_7);
			}
			...
			if($result_exam) {
				...
				$this->student_points_model->points_entry($idContact, $txt_test_ID, 'homework', $result_exam);
				$this->session->set_flashdata('first_view', true);
			}

			redirect('student/answer_homework/answer_homework_header/' . $subjectID, 'refresh');
		} else {

			redirect('student/answer_exam/answer_homework_header/' . $subjectID, 'refresh');
		} //end answer_homework
