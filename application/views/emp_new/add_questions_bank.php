  ...
  <div class="container">
    <div class="row">
      ...
      <main class="col-12 exam-box" style="padding: 30px;">
        <form class="form-ui" action="<?echo base_url('emp/Question_bank/saveBankQuestion');?>" method="post" id="my_form">
          <div class="exam-box__header">
            <div class="page-head success">
              <h1><?php
              $questionNames[2] = "إختر";
              $questionNames[1] = "إختر";
              $questionNames[3] = "صح أم خطاء";
              $questionNames[4] = " أكمل ";
              $questionNames[6] = " السوال المقالي ";
              $questionNames[7] = " التوصيل ";
              $questionNames[8] = " الرسم ";
              echo $questionNames[$get]; ?></h1>
            </div>
            <div class="exam-box__type">
              ...
              <button id="question_type_dropbtn" class="dropbtn">
                <div class="dropbtn-label">
                  ...
                  <?php echo $questionNames[$get]; ?>
                </div>
                <i class="far fa-chevron-down"></i>
              </button>
              <!-- HTML code -->
              <div id="myDropdown" class="dropdown-content">
    <?php foreach ($Type_question as $type) { ?>
        <a href="?id=<?php echo $type->ID; ?>&bankId=<?php echo $bankId; ?>&typeName=<?php echo $type->Name; ?>" class="dropdown-item">
            <?php echo $type->Name; ?>
        </a>
    <?php } ?>
</div>
          ...
          <input type="hidden" id="subjectid" name="BankId" value="<? echo $_GET['bankId']; ?>" />
          <input type="hidden" id="type" name="type" value="<?= $type; ?>" />
          <input type="hidden" id="get" name="get" value="<? echo $get; ?>" />
          <input type="hidden" id="Type_question_ID" name="Type_question_ID" value="<? echo $get; ?>"  />
            ...
            <div <?php if ($this->session->userdata('language') == 'english') { ?> style="margin-left: 222px;" <?php } else { ?> style="margin-right: 222px;" <?php } ?>>
              <label class="control-radio" style="font-size: large;">
                ...
                <input type="number" name="txt_Degree" value="<?php if (!$question_id) {
                                                              ...
                                                              } ?>" min="0.5" style="float:none;clear:both;width: -webkit-fill-available;" id="txt_Degree" step="0.5" required>
              </label>
            </div>
          </div>
          <?php if ($get == 2) { ?>
...
<script>
  $(document).ready(function() {
    ...
    $("#fileUpload").change(function(e) {
      if (!/\.(jpe?g|png|gif)$/i.test($('#fileUpload')[0].files[0].name)) {
        Swal.fire({
          icon: 'error',
          title: 'خطأ',
          text: ' الملف الذي تم إختيارة ليس صورة صالحة. الصيغ المدعومة هي: JPEG، PNG، أو GIF',
        });
        return;
      }
