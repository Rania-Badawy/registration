...
class Answer_Exam extends CI_Controller {
    ...
    function __construct()
    {
		...
		$this->load->model(array('emp/answer_exam_model','emp/answer_exam_new_model','emp/subject_model','emp/exam_model','emp/config_model'));
	...
	{
		         $Data['exam_details']    = $this->answer_exam_model->get_exams((string) $this->data['Lang'] );

				 $this->load->emp_template('answer_exam_list',$Data);
	}
//sees_answer
		...
		while($count_types<=7){
			...
			switch($txt_type){
				case'1':
					...
					$this->answer_exam_model->insert_s_degrees($q_old_id_1,$idContact);
				break;
				case'2':
				...
				while($count_answers<=$countRows){
					...
					//end check empty answer*/
					$this->answer_exam_model->insert_s_degrees($q_old_id_2,$idContact);
				break;
				case'3':
					...
					$this->answer_exam_model->insert_s_degrees($q_old_id_3,$idContact);
				break;
				case'4':
					$data            =  $this->answer_exam_model->get_degree($q_ID);
					$dataCorrect     =  $this->answer_exam_model->get_answer_correct_count($q_ID);
					...
					while($count_answers<=$countRows){
						...
						//end check empty answer*/
						$this->answer_exam_model->insert_s_degrees($q_old_id_4,$idContact);
				break;
				case'5':
					$data            =  $this->answer_exam_model->get_degree($q_ID);
					$dataCorrect     =  $this->answer_exam_model->get_answer_correct_count($q_ID);
					...
					while($count_answers<=$countRows){
						...
						//end check empty answer*/
						$this->answer_exam_model->insert_s_degrees($q_old_id_5,$idContact);
				break;
				case'6':
				    $case_6 =1;
