<?php
class Answer_Exercise_Model  extends CI_Model 
 {
///create_token
 ...
 //get_exams
	 public function get_exams($Lang)
	 {
	    ...
	    $this->db->join('vw_exercise_select AS vw_test', 'vw_test.row_level_ID = student.R_L_ID', 'INNER');	
 ...
 // 
	...
	public function get_sum_degree($TestID)
	{
		...
		SELECT test_Degree AS SumDegreeQ 
		 FROM vw_exercise_select WHERE test_ID = ".$TestID."
		");
	...
	/// get_sum_que
	public function get_sum_que($quetID)
	{
		$StudentID =(int)$this->session->userdata('id'); 
         
				$result2 = $this->db->query("SELECT SUM(test_student.Degree)AS SumDegreeSt ,  questions_content.Degree  FROM test_student 
				 inner JOIN questions_content ON questions_content.ID =  test_student.questions_content_ID 
				WHERE test_student.questions_content_ID =  ".$quetID." AND test_student.Contact_ID = ".$StudentID." 
				");	
				$NumRowResultExam = $result2->num_rows() ; 
				if($NumRowResultExam >0)
				  {	 
	...
	public function get_student_degree()
	{
		$StudentID =(int)$this->session->userdata('id'); 
				...
				$result2 = $this->db->query("SELECT SUM(test_student.Degree)AS Degree_student , CONCAT(tb1.Name,' ',tb2.Name) AS FullName , tb1.Img FROM test_student 
				 inner JOIN contact As tb1 on tb1.ID =  test_student.Contact_ID 
			 INNER JOIN student ON tb1.ID = student.Contact_ID
			 INNER JOIN contact AS tb2 ON student.Father_ID     = tb2.ID
				order by  Degree_student limit 10
				");	
				$NumRowResultExam = $result2->num_rows() ; 
				if($NumRowResultExam >0)
				  {	 
 ...
 //get_answers
	 public function get_answers($Lang)
	 {
		...
		$this->db->select('Answer_ID,Answer,Answer_correct,answers_IsActive,answers_Token,answers_Date_Stm,questions_content_ID,answers_attach');
			...
			foreach($id_q_degree as $id_q=>$degree)
			  {
				...
				if($id_q==$this->input->post('que_ID')){  $this->db->insert('test_student', $add_q);
		 add_notification( $this->db->insert_id(),8,0,0,0 ,$idContact);}
			      }else {
					$q_id_edit           = 	$id_q;	
					$ResultContactData = $ResultContactData ->result() ;
				...
				if($id_q==$this->input->post('que_ID')){ 	$Insert_q =  $this->db->update('test_student', $add_q);		}			  
			            }
			  }
	 ...
	 public function insert_upload_answer($txt_attach_f,$q_ID)
	 {
		...
		if($NumRowResultContactData <=0)
		  { 
			...
			$Insert_q =  $this->db->insert('test_student', $add_q);add_notification( $this->db->insert_id(),8,0,0,0 ,$idContact);
		}else {
			$ResultContactData = $ResultContactData ->result() ;
			foreach($ResultContactData as $row ){
			$question_s_id = $row->ID;
				   ...
				   foreach($ResultContactData as $row ){
						...
						if(!in_array($q_cntnt_ID , $q_id_edit)){
							  ...
							  if($NumRowResultContactDataS<=0){
								  ...
								  if($q_cntnt_ID==$this->input->post('que_ID')){  $Insert_q =  $this->db->insert('test_student', $add_q);add_notification( $this->db->insert_id(),8,0,0,0 ,$idContact);}
								  }else{
				...
				if($key==$item_answer){
					...
					if($NumRowResultContactData ==0)
					  { 
					   ...
					   $this->db->insert('test_student', $add_q);add_notification( $this->db->insert_id(),8,0,0,0 ,$idContact);
			      }else {
